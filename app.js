// ============================================================================
// MESHTASTIC PROTOBUF HANDLING
// ============================================================================

let MeshtasticProtobufs = null;
let FromRadioMessage = null;
let ToRadioMessage = null;
const ROOT_NAMESPACE = 'meshtastic';
...
/**
 * Загружает и подготавливает Protobuf-схему Meshtastic.
 */
function loadMeshtasticProto() {
    if (MeshtasticProtobufs) return true;
    try {
        // Проверяем, что protobuf.js загружен
        if (typeof protobuf === 'undefined' || !protobuf.Root) {
             logger.error('Protobuf.js library is missing! Make sure to include the script tag in index.html.');
             showToast('Protobuf Error: Library not found.', 'error');
             return false;
        }

        MeshtasticProtobufs = protobuf.Root.fromJSON(MESHTASTIC_PROTO_JSON);
        
        // Поиск необходимых типов сообщений
        FromRadioMessage = MeshtasticProtobufs.lookupType(`${ROOT_NAMESPACE}.FromRadio`);
        ToRadioMessage = MeshtasticProtobufs.lookupType(`${ROOT_NAMESPACE}.ToRadio`);

        logger.info('✓ Protobuf schema loaded and ready.');
        return true;
    } catch (error) {
        logger.error('Failed to load Protobuf schema. Check MESHTASTIC_PROTO_JSON for missing types/enums (e.g., HardwareModel).', error);
        showToast('Protobuf Init Error: Check MESHTASTIC_PROTO_JSON', 'error');
        return false;
    }
}

/**
 * Декодирует бинарный Protobuf payload (FromRadio) в JSON-объект.
 * @param {Uint8Array} buffer 
 */
function decodeFromRadio(buffer) {
    if (!FromRadioMessage) return null;
    try {
        // Декодирование и преобразование в простой JS-объект
        const message = FromRadioMessage.decode(buffer);
        // Используем { defaults: true } для получения всех полей, даже если они не установлены
        return message.toJSON({ defaults: true }); 
    } catch (error) {
        logger.error('FromRadio decoding failed (Corrupted data or incomplete schema):', error);
        // Логирование необработанных данных
        const hexData = Array.from(buffer).map(b => b.toString(16).padStart(2, '0')).join(' ');
        logger.error(`RAW Protobuf data that failed decoding: ${hexData}`);
        return null;
    }
}
...
const appState = {
  ...
  deviceInfo: {
    nodeId: '-',
    fwVersion: '-', // Adjusted to match index.html
    hwModel: '-', // Adjusted to match index.html
    region: '-',
    battery: '-',
    channelName: '-',
    nodeCount: 0,
...
const logger = {
  ...
  _log(level, message, obj = null) {
    ...
    if (obj) {
        messageContent += `<pre style="color: inherit; margin-top: 5px;">${JSON.stringify(obj, null, 2).substring(0, 500)}...</pre>`;
        // Log to browser console too
        if (level === 'ERROR' || level === 'WARN') {
            console.error(`[${level}] ${message}:`, obj);
        } else {
  ...
  debug(message, obj = null) { this._log('DEBUG', message, obj); },
  info(message, obj = null) { this._log('INFO', message, obj); },
  warn(message, obj = null) { this._log('WARN', message, obj); },
  error(message, obj = null) { this._log('ERROR', message, obj); },
};


function showToast(message, type = 'info') {
    const toastContainer = document.querySelector('.app-container');
    if (!toastContainer) return;
    
    // Check if element exists from previous session
...
function updateUIConnection() {
    ...
    statusEl.className = `status-badge ${appState.isConnected ? 'connected' : 'disconnected'}`;
    statusTextEl.textContent = appState.isConnected ? `Connected (${appState.connectionType.toUpperCase()})` : 'Disconnected';
    
    // Update button states
    const isUSB = appState.connectionType === 'usb';
    document.getElementById('serialConnectBtn').disabled = appState.isConnected;
    document.getElementById('serialDisconnectBtn').disabled = !isUSB;
    
    // Disable other connect buttons if already connected
...
function updateUIInfo() {
  // Connection Status (done in updateUIConnection)
  updateUIConnection();

  // Dashboard Info (matching IDs in user's index.html)
  ...
  // Other info like battery, channelName, nodeCount, uptime update here...

  // LoRa Config
  document.getElementById('txPowerValue').textContent = appState.loraConfig.txPower;
  
  // You would typically update select/range controls here if you received full config:
  // document.getElementById('txPower').value = appState.loraConfig.txPower;
  // document.getElementById('spreadingFactor').value = appState.loraConfig.spreadFactor;
  // document.getElementById('bandwidth').value = appState.loraConfig.bandwidth;
  
  // Message History (simplified for now)
  const messageList = document.getElementById('messageList');
  messageList.innerHTML = appState.messages.map(msg => 
...
// ============================================================================
// SERIAL CONNECTION (USB) - FIXED FOR BINARY PROTOCOL
// ============================================================================
...
const serialConnection = {
    ...
    // Meshtastic packets over USB have a 4-byte little-endian length prefix
    ...
    async connect() {
        ...
        if (!loadMeshtasticProto()) return; // Initialize Protobuf schema first
        
        try {
            // Request port (Meshtastic VID/PID filters can be added here)
            this.port = await navigator.serial.requestPort();

            // Open port at 115200 baud
            await this.port.open({ baudRate: 115200 });

            // Get raw binary streams
            ...
            showToast('Failed to connect USB.', 'error');
            this.disconnect();
        }
    },
    ...
    async readLoop() {
        while (this.isReading && this.port.readable) {
            try {
                ...
                if (value) {
                    // Append new binary data to buffer
        ...
        if (appState.isConnected) {
            this.disconnect(); // Disconnect if read loop closes unexpectedly
        }
    },

    // Combines new binary data with the existing buffer
    appendBuffer(newBytes) {
        const newBuffer = new Uint8Array(this.buffer.length + newBytes.length);
        newBuffer.set(this.buffer, 0);
        newBuffer.set(newBytes, this.buffer.length);
        this.buffer = newBuffer;
    },

    // Extracts full packets from the buffer based on the 4-byte length prefix
    processBuffer() {
        while (this.buffer.length >= this.LENGTH_PREFIX_SIZE) {
            // Read the 4-byte little-endian length prefix
            const dataView = new DataView(this.buffer.buffer);
            const payloadLength = dataView.getUint32(0, true); // true = little-endian

            const fullPacketSize = this.LENGTH_PREFIX_SIZE + payloadLength;

            if (this.buffer.length >= fullPacketSize) {
                // We have a full packet. Extract only the Protobuf payload.
                const payload = this.buffer.slice(this.LENGTH_PREFIX_SIZE, fullPacketSize);
                
                // Process the binary Protobuf payload
                this.handleData(payload);

                // Remove the processed packet from the buffer
                this.buffer = this.buffer.slice(fullPacketSize);
            } else {
                // Not enough data for the full packet, break and wait for more data
                break;
            }
        }
    },
    ...
    /**
     * Sends a binary Protobuf packet preceded by a 4-byte length prefix.
     * @param {Uint8Array} payload - Binary Protobuf data (ToRadio message).
     */
    async sendPacket(payload) {
        if (!this.writer) return;

        // Create buffer for [4-byte length prefix] + [Protobuf payload]
        const totalSize = this.LENGTH_PREFIX_SIZE + payload.length;
        const prefixedBuffer = new ArrayBuffer(totalSize);
        const dataView = new DataView(prefixedBuffer);
        
        // 1. Write the length of the payload (little-endian)
        dataView.setUint32(0, payload.length, true); // true = little-endian

        // 2. Copy the Protobuf payload after the prefix
        const payloadBytes = new Uint8Array(prefixedBuffer);
        payloadBytes.set(payload, this.LENGTH_PREFIX_SIZE);

        await this.writer.write(payloadBytes);
    },

    async sendWantConfigId() {
        if (!ToRadioMessage) return;

        logger.info('Sending WantConfigId to request node config via USB...');
        
        // Create a ToRadio message containing WantConfigId
        const toRadioMsg = { wantConfigId: {} };
        
        // Encode the message into binary Protobuf
        const buffer = ToRadioMessage.encode(toRadioMsg).finish();
        
        await this.sendPacket(buffer);
        logger.info('✓ WantConfigId sent, config requested.');
    },

    // Handle and decode incoming binary packet
    handleData(binaryPayload) {
        logger.debug(`Received Protobuf payload: ${binaryPayload.length} bytes. RAW: ${Array.from(binaryPayload).map(b => b.toString(16).padStart(2, '0')).join(' ')}`);
        
        const decoded = decodeFromRadio(binaryPayload);

        if (decoded) {
            // 1. PROCESS NODE INFO
            if (decoded.nodeInfo) {
                logger.info('Extracted NodeInfo/Config data.');
                
                // Convert node ID to !HEX format
                const nodeIdHex = decoded.nodeInfo.num.toString(16).toUpperCase().padStart(8, '0');
                appState.deviceInfo.nodeId = `!${nodeIdHex}`;
                appState.deviceInfo.fwVersion = decoded.nodeInfo.firmwareVersion || '-'; // Matching #fwVersion
                appState.deviceInfo.hwModel = decoded.nodeInfo.hwModel || '-'; // Matching #hwModel
                // Additional info can be extracted here
            }
            
            // 2. PROCESS LORA CONFIG
            if (decoded.loraConfig) {
                // Note: The UI requires the full LoraConfig to update all fields correctly.
                appState.loraConfig.txPower = decoded.loraConfig.txPower || '-';
                appState.loraConfig.spreadFactor = decoded.loraConfig.spreadFactor || '-';
                appState.loraConfig.bandwidth = decoded.loraConfig.bandWidth || '-';
                appState.loraConfig.frequency = decoded.loraConfig.channelNum ? `Channel ${decoded.loraConfig.channelNum}` : 'Decoded'; 
            }
            
            // 3. PROCESS TEXT MESSAGES
            if (decoded.packet && decoded.packet.decoded && decoded.packet.decoded.portnum === 1) { // 1 = TEXT_MESSAGE_APP
                const payloadBytes = decoded.packet.decoded.payload;
                
                // Use built-in TextDecoder for reliable UTF-8 decoding
                const textMessage = new TextDecoder().decode(payloadBytes);
                const senderIdHex = decoded.packet.from.toString(16).toUpperCase().padStart(8, '0');
                ...
                logger.info(`Decrypted Text Message from !${senderIdHex}: "${textMessage}"`);
                showToast(`New message from !${senderIdHex}`, 'info');
            }
            
            updateUIInfo();

        } else {
          logger.warn('Protobuf payload failed decoding or was empty.');
        }
    },

    async sendMessage(text) {
        if (!ToRadioMessage || !this.writer) {
            showToast('Not connected or Protobuf not ready.', 'error');
            return;
        }
        
        logger.info(`Sending USB message: "${text}"`);

        try {
            // Encode text into binary payload
            const payloadBytes = new TextEncoder().encode(text);
            
            // Create Protobuf ToRadio message
            const toRadioMsg = {
                packet: {
                    decoded: {
            ...
            logger.info('✓ USB message sent');
            showToast('Message sent!', 'success');
        } catch (error) {
            logger.error('USB send failed', error.message);
    ...
    async disconnect() {
        ...
        try {
            if (this.reader) {
                // Cancel pending read() operation and release the lock
...
// (Using the user's provided structure for simplicity)
// ============================================================================
...
function setupEventListeners() {
    ...
    document.querySelector('.tabs').addEventListener('click', (e) => {
        if (e.target.classList.contains('tab-button')) {
            ...
            document.querySelectorAll('.app-container > .tab-content').forEach(content => content.classList.remove('active'));
    ...
    // Connection sub-tabs
    document.querySelector('#connection > .tabs').addEventListener('click', (e) => {
        if (e.target.classList.contains('tab-button')) {
    ...
    document.getElementById('serialConnectBtn').addEventListener('click', () => serialConnection.connect());
    document.getElementById('serialDisconnectBtn').addEventListener('click', () => serialConnection.disconnect());

    // Message Sending
    document.getElementById('sendMessageBtn').addEventListener('click', () => {
        const messageTextarea = document.getElementById('messageText');
        const text = messageTextarea.value.trim();
        if (text && appState.isConnected && appState.connectionType === 'usb') {
            serialConnection.sendMessage(text);
            messageTextarea.value = '';
        } else if (!appState.isConnected) {
            showToast('Not connected. Connect via USB first.', 'warning');
        }
    });
    
    // Log filter
    document.getElementById('logLevelFilter').addEventListener('change', (e) => {
        logger.logLevel = e.target.value;
        // Simple re-render of logs (clearing for demo)
        document.getElementById('logConsole').innerHTML = ''; 
        logger.info(`Log level set to: ${logger.logLevel}`);
    });
    
    // Dummy button handlers (for completeness)
...
document.addEventListener('DOMContentLoaded', () => {
  // Check for protobuf.js dependency (since the user didn't provide index.html's head)
  if (typeof protobuf === 'undefined') {
    alert("FATAL ERROR: protobuf.js library is missing. Please add <script src='https://cdn.jsdelivr.net/npm/protobufjs@7.2.5/dist/protobuf.min.js'></script> to your index.html.");
    // Force logger init just to catch errors
    logger.error('FATAL: Protobuf.js library is missing. Cannot proceed.');
  } else {
    // Attempt to load Protobuf schema
    loadMeshtasticProto();
  }
  ...
  if (!('serial' in navigator)) {
      showToast('Ваш браузер не поддерживает Web Serial API (USB). Используйте Chrome или Edge.', 'error');
      document.getElementById('serialConnectBtn').disabled = true;
  }
});

// Since the user is focused on USB, we remove the broken/incomplete BLE/TCP stubs
// to keep the code clean and focused on the required function.
// For any other connection type (BLE/TCP), the relevant functions must be re-implemented.